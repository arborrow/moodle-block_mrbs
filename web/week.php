<?php
# $Id: week.php,v 1.11 2009/06/10 15:07:11 mike1989 Exp $
...
if($num_of_days==0){$num_of_days=7;}//if user has not configured this, default to 7
...
if ( $pview != 1 ) {
  ...
  if ($area_list_format == "select") {
    ...
    $this_room_description = sql_query1("select description from $tbl_room where id=$room");
  } else {
  ...
  if ($area_list_format == "select") {
    ...
    if ($res) for ($i = 0; ($row = sql_row($res, $i)); $i++)
    {
        ...
        if ($row[0] == $room)
        {
            ...
            $this_room_description = htmlspecialchars($row[2]);
            if ( $pview != 1 )
