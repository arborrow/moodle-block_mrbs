...
if($type == "room") {
    ...
    if(isset($confirm))	{
        // Todo: switch to use Moodle delete_record(s), follow up on transaction-like
...
if($type == "area") {
    // We are only going to let them delete an area if there are no rooms. its easier
    $n = sql_query1("select count(*) from $tbl_room where area_id=$area");
    if ($n == 0) { // OK, nothing there, lets blast it away
