...
if (($day==0) or ($month==0) or ($year==0)) { //If we dont know the right date then make it up
    $day   = date("d");
    $month = date("m");
    $year  = date("Y");
} else { // Make the date valid if day is more then number of days in month
...
/* ensure that $morningstarts_minutes defaults to zero if not set not needed if initialized with optoinal_param - ab.
...
if ($pview!= 1) {
   ...
   echo "<u>".get_string('areas','block_mrbs')."</u><br>"; //Show all avaliable areas

   // need to show either a select box or a normal html list, depending on the settings in config.inc.php
   ...
   //insert the goto room form
    echo "<td width=\"20%\"><h3>Find a room</h3><form action='gotoroom.php' method='get'>";
    echo "Go to:<input name='room'>";
...
if (!empty($area)) {
	...
	for ($t = $start_t; $t <= $end_t; $t += $resolution) {
            if (empty($today[$row[0]][date($format,$t)])) { //checks for double bookings
    ...
    //TODO: change to use Moodle debugging settings instead
    if ($debug_flag) {
       echo '<p>DEBUG:<pre>\n';
    ...
    //TODO: Switch to Moodle get_record(s)
    $sql = "select room_name, capacity, id, description from $tbl_room where area_id=$area order by 1";

    $res = sql_query($sql);

    // It might be that there are no rooms defined for this area.
    // If there are none then show an error and dont bother doing anything else
    ...
    if (sql_count($res) == 0) {
        ...
        sql_free($res);
            ...
            if ( $pview != 1 ) { //Show Go to day before and after links
            ...
            if ($javascript_cursor) { // If authorized in config.inc.php, include the javascript cursor management.
                ...
                echo "<SCRIPT language=\"JavaScript\">InitActiveCell(". ($show_plus_link ? "true" : "false") . ", "."true, "
            ...
            echo "<table cellspacing=0 border=1 width=\"100%\">"; //This is where we start displaying stuff
		...
		$rooms[] = $row[2];
            ...
            if (FALSE != $times_right_side) {
                echo "<th width=\"1%\">".($enable_periods  ? get_string('period','block_mrbs') : get_string('time'))."</th>";
            }
            echo "</tr>\n";
		...
		echo "<tr>";
                //TODO: Explore how to make better themeable with CSS
		tdcell("red");
		if( $enable_periods ){
		...
		while (list($key, $room) = each($rooms)) {
			...
			// If the room isnt booked then allow it to be booked
                    ...
                    for ($i=0;$i<count($descrs);$i++) {
                        if ($descrs[$i] != "") { //if it is booked then show
