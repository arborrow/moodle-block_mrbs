<?php
// $Id: postgres7.php,v 1.1 2008/08/09 18:32:57 arborrow Exp $
...
$db_port= strpos($db_host,':');

if ($db_port) {
    $db_port_name = substr($db_host, $db_port+1);
    $db_host = substr($db_host, 0, $db_port);
    $conninfo = (empty($db_host) ? "" : "host=$db_host port=$db_port_name ")."dbname=$db_database user=$db_login password=$db_password";
}
if (!isset($conninfo)) {
    $conninfo = (empty($db_host) ? "" : "host=$db_host ")."dbname=$db_database user=$db_login password=$db_password";
}
